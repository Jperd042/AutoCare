...
export default function Dashboard({ account, navigation, route, onSignOut, onSaveProfile, onDeleteAccount }) {
  ...
  const [profileSection, setProfileSection] = useState(route?.params?.profileSection || 'rewards');
  ...
  useEffect(() => {
    ...
    if (route?.params?.initialMenuScreen) {
      setMenuScreen(route.params.initialMenuScreen);
    }

    if (route?.params?.bookingMode) {
      setBookingMode(route.params.bookingMode);
    }

    if (route?.params?.profileSection) {
      setProfileSection(route.params.profileSection);
    }
  }, [
    route?.params?.initialMenuScreen,
    route?.params?.initialTab,
    route?.params?.bookingMode,
    route?.params?.profileSection,
  ]);
  ...
  useEffect(() => {
    ...
    const backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      ...
      if (activeTab === 'menu') {
        if (menuScreen === 'security' || menuScreen === 'personal') {
          setMenuScreen('settings');
          return true;
        }

        if (menuScreen === 'settings' || menuScreen === 'gift' || menuScreen === 'saved') {
          setMenuScreen('root');
          return true;
        }
      }
  ...
  const renderScrollableContent = (contentStyle, children) => {
    ...
    const adjustedContentStyle = isWeb
      ...
      : [
          contentStyle,
          {
            paddingTop: (flattenedContentStyle.paddingTop || 0) + insets.top,
          },
        ];
    ...
    if (isWeb) {
      return (
        <View style={styles.scrollRegion}>
          <View style={[styles.webScrollContent, adjustedContentStyle]}>{children}</View>
        </View>
      );
    }
    ...
    return (
      <ScrollView
        ...
        contentContainerStyle={adjustedContentStyle}
            ...
            >
              ...
              >
                <View style={styles.productHero}>
                  ...
                  <TouchableOpacity
                    style={[styles.productBackButton, { top: PRODUCT_BACK_BUTTON_TOP + insets.top }]}
